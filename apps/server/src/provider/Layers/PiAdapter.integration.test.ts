  ...
  it.layer(integrationLayer)("runs a full turn", (it) => {
    it.effect("runs a full turn against the real pi RPC process", () =>
      Effect.gen(function* () {
        ...
        const adapter = yield* makePiAdapter(decodePiSettings({ binaryPath: "pi" })).pipe(
          Effect.orDie,
        );
    ...
    it.effect(
      "aborting mid-turn stops pi and emits turn.aborted",
      () =>
        Effect.gen(function* () {
          const inventory = yield* probePiInventory({
            binaryPath: "pi",
          ...
          const threadId = ThreadId.make("pi-integration-interrupt");
          const tap = yield* makeEventTap(adapter);
          ...
          yield* adapter.sendTurn({
            threadId,
            input:
              "Use the bash tool to run exactly: sleep 60. Do not explain, just run it and wait.",
            modelSelection: { instanceId: ProviderInstanceId.make("pi"), model: modelSlug },
          });

          // Wait until the model has actually started streaming, then interrupt.
          yield* tap.waitFor(
          ...
          assert.equal(completed.type, "turn.aborted");
